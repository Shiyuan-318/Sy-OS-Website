...
<body>
    ...
    <section class="page-content">
        <div class="page-image-card" data-animate>
            <img src="{img}" alt="{title} 详细展示" onerror="this.onerror=null;this.src='1145.png';">
        </div>
        <div class="section-header" data-animate>
            <span class="section-eyebrow">FEATURES</span>
            <h2 class="section-title">功能详情</h2>
            <p class="section-subtitle">{detail}</p>
        </div>
        <div class="detail-tags" data-animate>
            {tags_html}
        </div>
        <p class="detail-text" data-animate>{title} 更多功能与介绍正在持续更新中，敬请期待。</p>
    </section>
...
for p in pages:
    tags_html = "\n".join(
        f'            <span class="product-tag">{t}</span>' for t in p["tags"].split(",")
    )
    content = template.replace("{title}", p["title"]).replace("{desc}", p["desc"])
    content = content.replace("{img}", p["img"]).replace("{eyebrow}", p["eyebrow"])
    content = content.replace("{detail}", p["detail"]).replace("{tags_html}", tags_html)
    path = os.path.join(DIR, p["file"])
